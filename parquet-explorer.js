...
function parquetToast(title, icon = 'info') { if (window.Swal) Swal.fire({ title, icon, toast: true, position: 'top-end', timer: 2200, showConfirmButton: false }); }
function activeParquetTab() { return parquetState.tabs.find(t => t.id === parquetState.activeTabId) || null; }
...
function createParquetUI() {
  ...
  section.querySelector('.inline-link').onclick = choose;
  section.querySelector('#parquet-open-btn').onclick = choose;
  section.querySelector('#parquet-add-tab').onclick = choose;
  section.querySelector('#parquet-export-btn').onclick = exportParquetCsv;
  input.onchange = e => { const files = [...e.target.files]; if (files.length) openParquetFiles(files); input.value = ''; };
  ...
  dz.addEventListener('drop', e => { const files = [...e.dataTransfer.files].filter(f => /\.parq(uet)?$/i.test(f.name)); if (files.length) openParquetFiles(files); });
...
async function openParquetFiles(files) {
  const available = parquetState.maxTabs - parquetState.tabs.length;
  if (available <= 0) return parquetToast('Maximum of 10 Parquet tabs reached', 'info');
  const selected = files.filter(f => /\.parq(uet)?$/i.test(f.name)).slice(0, available);
  if (!selected.length) return parquetToast('Please select Parquet files', 'warning');
  if (files.length > available) parquetToast(`Only ${available} more file${available === 1 ? '' : 's'} can be added`, 'info');
  ...
  try {
    ...
    for (let i = 0; i < selected.length; i++) {
      document.getElementById('parquet-loading-title').textContent = `Opening ${i + 1} of ${selected.length}…`;
      document.getElementById('parquet-loading-text').textContent = selected[i].name;
      await addParquetTab(db, selected[i]);
    }
    renderParquetTabs();
    await activateParquetTab(parquetState.tabs[parquetState.tabs.length - selected.length].id);
    parquetToast(`${selected.length} Parquet file${selected.length === 1 ? '' : 's'} opened`, 'success');
  } catch (err) {
    console.error(err); parquetToast(`Could not open file: ${err.message || err}`, 'error');
  } finally { loading.hidden = true; drop.hidden = parquetState.tabs.length > 0; empty.hidden = parquetState.tabs.length > 0; }
}

async function addParquetTab(db, file) {
  const duplicate = parquetState.tabs.find(t => t.fileName === file.name);
  const suffix = duplicate ? `-${Date.now()}` : '';
  const id = `parquet_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;
  const path = `/tmp/${id}-${file.name.replace(/[^a-zA-Z0-9._-]/g, '_')}`;
  await db.registerFileBuffer(path, new Uint8Array(await file.arrayBuffer()));
  const viewName = `dataset_${id.replace(/[^a-zA-Z0-9_]/g, '_')}`;
  await parquetState.conn.query(`CREATE OR REPLACE VIEW ${parquetEscapeIdent(viewName)} AS SELECT * FROM read_parquet(${parquetEscapeString(path)})`);
  const count = await parquetState.conn.query(`SELECT COUNT(*)::BIGINT AS n FROM ${parquetEscapeIdent(viewName)}`);
  const schema = await parquetState.conn.query(`DESCRIBE ${parquetEscapeIdent(viewName)}`);
  parquetState.tabs.push({ id, fileName: file.name, displayName: file.name.replace(/\.parq(uet)?$/i, '') || file.name, path, viewName, page: 0, filter: '', totalRows: Number(count.toArray()[0].n), columns: schema.toArray().map(r => ({ name: r.column_name, type: r.column_type, nullable: r.null })) });
}

function renderParquetTabs() {
  const bar = document.getElementById('parquet-file-tabs');
  const add = document.getElementById('parquet-add-tab');
  bar.querySelectorAll('.parquet-file-tab').forEach(el => el.remove());
  parquetState.tabs.forEach(tab => {
    const button = document.createElement('button'); button.className = `parquet-file-tab${tab.id === parquetState.activeTabId ? ' active' : ''}`; button.dataset.id = tab.id; button.title = 'Double-click to rename';
    button.innerHTML = `<i class="fas fa-table"></i><span class="tab-name">${escapeParquetHtml(tab.displayName)}</span><button class="tab-close" title="Close tab"><i class="fas fa-xmark"></i></button>`;
    button.onclick = e => { if (!e.target.closest('.tab-close')) activateParquetTab(tab.id); };
    button.ondblclick = e => { if (!e.target.closest('.tab-close')) openParquetRename(tab.id, button); };
    button.querySelector('.tab-close').onclick = e => { e.stopPropagation(); closeParquetTab(tab.id); };
    bar.insertBefore(button, add);
  });
  add.disabled = parquetState.tabs.length >= parquetState.maxTabs;
  document.getElementById('parquet-tabs-wrap').hidden = parquetState.tabs.length === 0;
}

async function activateParquetTab(id) {
  const tab = parquetState.tabs.find(t => t.id === id); if (!tab) return;
  parquetState.activeTabId = id; renderParquetTabs();
  document.getElementById('parquet-app').hidden = false;
  document.getElementById('parquet-file-name').textContent = tab.displayName;
  document.getElementById('parquet-file-meta').textContent = `${tab.totalRows.toLocaleString()} rows · ${tab.columns.length} columns · ${tab.fileName}`;
  ...
  document.getElementById('parquet-schema-list').innerHTML = tab.columns.map(c => `<div class="schema-item"><i class="fas fa-${/INT|DOUBLE|DECIMAL|FLOAT|BIGINT|SMALLINT|TINYINT/i.test(c.type) ? 'hashtag' : /DATE|TIME|TIMESTAMP/i.test(c.type) ? 'calendar' : 'font'}"></i><span>${escapeParquetHtml(c.name)}</span><span class="schema-type">${escapeParquetHtml(c.type)}</span></div>`).join('');
  document.getElementById('parquet-schema-details').innerHTML = `<div class="schema-detail-row header"><span>Column</span><span>Type</span><span>Nullable</span></div>` + tab.columns.map(c => `<div class="schema-detail-row"><span>${escapeParquetHtml(c.name)}</span><span>${escapeParquetHtml(c.type)}</span><span>${escapeParquetHtml(c.nullable || 'YES')}</span></div>`).join('');
  await renderParquetPage();
}

async function renderParquetPage() {
  const tab = activeParquetTab(); if (!tab) return;
  const where = tab.filter ? ` WHERE ${tab.filter}` : '';
  const total = await parquetState.conn.query(`SELECT COUNT(*)::BIGINT AS n FROM ${parquetEscapeIdent(tab.viewName)}${where}`);
  const filteredTotal = Number(total.toArray()[0].n);
  const offset = tab.page * parquetState.pageSize;
  const result = await parquetState.conn.query(`SELECT * FROM ${parquetEscapeIdent(tab.viewName)}${where} LIMIT ${parquetState.pageSize} OFFSET ${offset}`);
  const rows = result.toArray(); const names = tab.columns.map(c => c.name);
  document.querySelector('#parquet-table thead').innerHTML = `<tr>${names.map(n => `<th>${escapeParquetHtml(n)}</th>`).join('')}</tr>`;
  document.querySelector('#parquet-table tbody').innerHTML = rows.length ? rows.map(r => `<tr>${names.map(n => { const value = formatParquetValue(r[n]); return `<td title="${escapeParquetHtml(value)}">${escapeParquetHtml(value)}</td>`; }).join('')}</tr>`).join('') : `<tr><td colspan="${Math.max(1,names.length)}" style="text-align:center;padding:45px;color:#7b8493">No rows match this filter.</td></tr>`;
...
async function applyParquetFilter() { const tab = activeParquetTab(); if (!tab) return; tab.filter = document.getElementById('parquet-filter').value.trim(); tab.page = 0; try { await renderParquetPage(); } catch (err) { parquetToast(`Invalid filter: ${err.message || err}`, 'error'); } }
...
function switchParquetView(view) { document.querySelectorAll('.parquet-subtab').forEach(t => t.classList.toggle('active', t.dataset.view === view)); document.getElementById('parquet-data-view').classList.toggle('active', view === 'data'); document.getElementById('parquet-schema-view').classList.toggle('active', view === 'schema'); }

function openParquetRename(id, anchor) {
  document.querySelector('.tab-rename-popover')?.remove();
  const tab = parquetState.tabs.find(t => t.id === id); if (!tab) return;
  const pop = document.createElement('div'); pop.className = 'tab-rename-popover';
  pop.innerHTML = `<input value="${escapeParquetHtml(tab.displayName)}" maxlength="80"><button class="save">Rename</button><button class="cancel">Cancel</button>`;
  document.body.appendChild(pop);
  const rect = anchor.getBoundingClientRect(); pop.style.left = `${Math.min(rect.left, window.innerWidth - 360)}px`; pop.style.top = `${rect.bottom + 6}px`;
  const input = pop.querySelector('input'); input.focus(); input.select();
  const save = () => { const name = input.value.trim(); if (name) { tab.displayName = name; if (parquetState.activeTabId === id) document.getElementById('parquet-file-name').textContent = name; renderParquetTabs(); } pop.remove(); };
  pop.querySelector('.save').onclick = save; pop.querySelector('.cancel').onclick = () => pop.remove(); input.onkeydown = e => { if (e.key === 'Enter') save(); if (e.key === 'Escape') pop.remove(); };
  setTimeout(() => document.addEventListener('mousedown', function close(e) { if (!pop.contains(e.target) && e.target !== anchor) { pop.remove(); document.removeEventListener('mousedown', close); } }), 0);
}

async function closeParquetTab(id) {
  const index = parquetState.tabs.findIndex(t => t.id === id); if (index < 0) return;
  parquetState.tabs.splice(index, 1);
  if (parquetState.activeTabId === id) parquetState.activeTabId = parquetState.tabs[index]?.id || parquetState.tabs[index - 1]?.id || null;
  if (parquetState.tabs.length) await activateParquetTab(parquetState.activeTabId); else { document.getElementById('parquet-app').hidden = true; document.getElementById('parquet-tabs-wrap').hidden = true; document.getElementById('parquet-dropzone').hidden = false; document.getElementById('parquet-empty-state').hidden = false; }
  renderParquetTabs();
}

async function exportParquetCsv() {
  const tab = activeParquetTab(); if (!tab || !parquetState.conn) return parquetToast('Open a Parquet file first', 'info');
  try {
    const where = tab.filter ? ` WHERE ${tab.filter}` : '';
    const exportPath = `/tmp/${tab.id}-export.csv`;
    await parquetState.conn.query(`COPY (SELECT * FROM ${parquetEscapeIdent(tab.viewName)}${where}) TO ${parquetEscapeString(exportPath)} (HEADER, DELIMITER ',')`);
    const csv = await parquetState.db.copyFileToBuffer(exportPath);
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `${tab.displayName.replace(/[^a-zA-Z0-9._-]/g, '_')}.csv`; a.click(); setTimeout(() => URL.revokeObjectURL(url), 1000); parquetToast('CSV exported', 'success');
  } catch (err) { parquetToast(`Export failed: ${err.message || err}`, 'error'); }
}
